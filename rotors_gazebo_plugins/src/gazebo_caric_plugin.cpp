...
namespace gazebo
{
    ...
    void GazeboCaricPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
    {
        ...
        for (int i=0; i<idListener_.size(); i++)
        {
            ...
            ros::Publisher view_pub = ros_node_handle2_->advertise<visualization_msgs::Marker>
                                    ("/firefly"+std::to_string(i+1)+"/visualize", 1);
...
//   ROS_INFO("start odomCB 1 for robot %d!!", getId());
  if (odom_deque.size()>odom_save_size_) odom_deque.pop_front();
...
void GazeboCaricPlugin::Listener::checkEvaluateCaptures()
{
  ...
  for (int i = 0; i < trigger_deque.size(); i++)
  {
    ...
    if (!findClosestOdom(trigger_deque[i], odom)) continue;
